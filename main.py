...
#set the window size, width and height in pixels
turtle.setup(500, 600)

# define starting coordinates for the faces
...
# function to draw each part of the face
def draw_face(face_detailes_list, start_point):
    turtle.penup()
    ...
    for i in range(len(face_detailes_list[0])):
        x, y = face_detailes_list[0][i]
        turtle.goto(x, y)

    for i in range(len(face_detailes_list[1])):
        x, y = face_detailes_list[1][i]
        turtle.goto(x, y)
